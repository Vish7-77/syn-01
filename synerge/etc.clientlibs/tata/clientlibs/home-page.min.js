    ...
    for (var i = 0; i < carItem.length; i++) {
      if (carItem[i].children[0].classList.contains("videoContainerdiv")) {
        ...
        playerInstance.on("time", function (e) {
          ...
          var playerMarkerIndex = findObjectIndexById(
            playersMarkers,
            "id",
            this.id
          );
          if (
            ...
            playersMarkers[playerMarkerIndex].markers.indexOf(percentPlayed) ==
              -1
          ) {
            playersMarkers[playerMarkerIndex].markers.push(percentPlayed);
            addToDataLayerWithPct(src, "Progress", percentPlayed);
          } else if (
            markers.indexOf(percentPlayedupper) > -1 &&
            playersMarkers[playerMarkerIndex].markers.indexOf(
              percentPlayedupper
            ) == -1
          ) {
...
xhttp.onload = function () {
  if (xhttp.readyState == 4 && xhttp.status === 200) {
    ...
    for (var i = 0; i < res.response.length; i++) {
      ...
      if (res.response[i].type == "twitter") {
...
var carousel = document.getElementsByClassName(
  "carouselIndicators nIndicators"
);
...
function twitterCardClickable(event) {
  ...
  if (
    event.target.classList.contains("newsFeedA") ||
    event.target.classList.contains("feedsTwitter") ||
    event.target.classList.contains("tweet-date")
  ) {
    var url = document
      .querySelector(".twitterFeed .carItem.active .twitterContent>a")
      .getAttribute("href");
    window.open(url);
  } else {
...
let twitterCardJobs = [...document.querySelectorAll(".twitterFeed")].filter(
  (x) => x.querySelector(".jobsCarouselHome")
);
twitterCardJobs.map((x) =>
...
if (window.innerWidth > 991) {
  document.addEventListener("DOMContentLoaded", () => {
    ...
    for (let elem of linePointers) {
      setTimeout(() => {
        elem.classList.remove("artificialHover");
      }, 5000);
    }
...
fullCardClick.addEventListener("click", function (e) {
  if (e.target.classList.contains("playVideo")) {
    ...
    let direction = "_self";
    let toGetUrl = document
      .getElementsByClassName("CardUrl")[0]
      .getAttribute("href");
